...
import { submitAcademicsData } from '../API/contorller/educationalController'; 
...
const EducationalBackground = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call the API to submit the qualifications
      await submitAcademicsData(qualifications);
      toast.success("Qualifications submitted successfully!");

      // Reset qualifications after successful submission
      setQualifications([{ qualification: '', institution: '', percentage: '', passingYear: '', country: '' }]);
    } catch (error) {
    ...
    <section className="form-section">
      ...
      <form onSubmit={handleSubmit}> {/* Wrap fields in a form */}
        ...
        <button type="submit" onClick={handleSubmit} disabled={loading}> {/* Submit button */}
          {loading ? "Submitting..." : "Submit Qualifications"}
        </button>
      </form>
    </section>
  );
